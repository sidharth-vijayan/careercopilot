...
import { updateApplicationStatus, deleteApplication as deleteAppAction } from "@/actions/application";
...
export function ApplicationsClient({ initialApplications }: ApplicationsClientProps) {
  ...
  const handleStatusUpdate = async (id: string, status: ApplicationStatus) => {
    ...
    if (!res.success) {
      toast("Error", { description: res.error, type: "error" });
  ...
  const handleRefresh = () => {
    // A real app might re-fetch from the server, but for now we rely on the modal onSuccess callback to trigger a page reload or state update.
    // Easiest is to force a page reload to get fresh server data.
