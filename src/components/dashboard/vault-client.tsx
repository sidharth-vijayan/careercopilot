...
export function VaultClient({ initialItems }: { initialItems: VaultItem[] }) {
  // Seeded by the server component, so the list is populated on first paint
  // instead of after a client round-trip. Refetches happen from event handlers.
    ...
    if (response.success && response.data) {
      setItems(
        response.data.map((item) => ({
          id: item.id,
          // The action returns `type` as a plain string; the UI narrows it.
          ...
          {filteredItems.length === 0 ? (
            <div className="rounded-xl border border-dashed p-16 text-center bg-card">
              ...
              <p className="text-muted-foreground mt-2 max-w-sm mx-auto text-sm">
                Add your historic credentials, or click &ldquo;Load B2B Demo Vault&rdquo; to prefill realistic B2B SaaS developer details.
              </p>
